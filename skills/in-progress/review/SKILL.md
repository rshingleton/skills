---
name: review
description: Review the changes since a fixed point (commit, branch, tag, or merge-base) along two axes — Standards (does the code follow this repo's documented coding standards?) and Spec (does the code match what the originating issue or Epic asked for?). Runs both reviews in parallel sub-agents and reports them side by side. Use when the user wants to review a branch, a PR, work-in-progress changes, or asks to "review since X".
---
...
- **Spec** — does the code faithfully implement the originating issue, Epic, or plan spec?

Both axes run as **parallel sub-agents** so they don't pollute each other's context, then this skill aggregates their findings.

The issue tracker should have been provided to you — run `/setup-internal-skills` if `docs/agents/issue-tracker.md` is missing.
...
3. An Epic (`docs/issues/…/epic.md`), plan spec, or file under `specs/` or legacy `.scratch/` matching the branch or feature.
