...
**Jira:** copy [`.env.example`](../.env.example) to `~/.config/ai-skills/.env` or the app repo `.env`; `source` [load-jira-env.sh](../scripts/load-jira-env.sh) before API calls. Optional `JIRA_ASSIGNEE`, `JIRA_WATCHER_IGNORE` (remove) / `JIRA_WATCHER_USERNAME` (add) / `JIRA_EMAIL` (PAT fallback); all writes use [jira-notifications.md](../skills/engineering/setup-internal-skills/jira-notifications.md). Creates use [jira-description-style.md](../skills/engineering/setup-internal-skills/jira-description-style.md).
