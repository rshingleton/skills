...
**ai-skills** monorepo — composable `SKILL.md` workflows for planning, implementation, audit, verify, and baseline codebase reference (`/doc-it`). Forked from [mattpocock/skills](https://github.com/mattpocock/skills); the Doc Cycle, Jira integration, and `-it` skill family here are original to this fork.

**Repository:** `https://github.com/rshingleton/skills.git` (default branch `main`)

Install into a machine:
...
1. Every shipped skill in `engineering/` or `productivity/` (the **promoted** buckets) needs:
   - A linked entry in the top-level [README.md](README.md)
   - An entry in [.claude-plugin/plugin.json](.claude-plugin/plugin.json)'s `skills` array (the Claude Code plugin ships exactly the promoted set) — run `claude plugin validate . --strict` after touching it
   - A human-facing docs page at `docs/<bucket>/<skill-name>.md` (see [.agents/writing-docs.md](.agents/writing-docs.md) for the template)
   - A mention in the [ask-matt](skills/engineering/ask-matt/SKILL.md) router if it's user-reachable, so the map stays accurate
   - Skills in `misc/`, `in-progress/`, and `deprecated/` must **not** appear in the README, plugin manifest, or get a docs page
...
5. No em-dashes anywhere in this repo's prose (`SKILL.md` files, docs, `README.md`, `CHANGELOG.md`, ADRs, code comments) — rewrite with a comma, colon, period, parentheses, or conjunction instead.
...
**Intake:** `/issue-it` (capture local or paste email/ServiceNow) · audit skills → [audit-to-issues.md](skills/engineering/setup-internal-skills/audit-to-issues.md). Plan: `/plan-it --from-issues` (intake evaluation + planning in one session).

**Plan:** `/plan-it` (ad-hoc or `--from-issues`) — **always grills** before scaffold; optional triage pre-work does not replace the grill.

**Doc Cycle:** `/plan-it` → `/implement-it` (each phase) → `/audit-it` (once, or `--phase` per phase) → `/verify-it` (once, or `--phase` per phase) → `/commit-it` (stage, commit, PR).
...
**Email / ServiceNow:** paste in project repo → `/issue-it` → review inbox → `/plan-it --from-issues` — [SCENARIO-EMAIL-SERVICENOW.md](skills/engineering/setup-internal-skills/SCENARIO-EMAIL-SERVICENOW.md).
...
| [setup-internal-skills/scripts/load-jira-env.sh](skills/engineering/setup-internal-skills/scripts/load-jira-env.sh) | Load Jira vars + helpers from `.env` (canonical; also sources `jira-helpers.sh`) |
| [setup-internal-skills/scripts/jira-helpers.sh](skills/engineering/setup-internal-skills/scripts/jira-helpers.sh) | All Jira shell helper functions (auto-sourced by `load-jira-env.sh`) |
